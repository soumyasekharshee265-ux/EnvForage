...
def test_validate_pydantic_errors():
    ...
    try:
        ...
        assert any(
            "os_support" in err and "Invalid os_support" in err for err in errors
        )
    finally:
        tmp_path.unlink()
...
def test_logical_error_cuda_required_without_versions():
    ...
    try:
        ...
        assert is_valid is False
        assert any(
            "cuda_required is True, but cuda_versions is empty" in err for err in errors
        )
    finally:
        tmp_path.unlink()
...
def test_logical_error_cuda_variant_mismatch():
    ...
    try:
        ...
        assert is_valid is False
        assert any(
            "Package 'torch' specifies cuda_variant 'cu124' (mapped to '12.4'), which is not compatible"
            in err
            for err in errors
        )
    finally:
